...
public class NodeStoreTree {
    ...
    public NodeStoreTree(ReadOnlyStore store, boolean skipSizeCheck) {
        this.store = store;
        ...
        DefaultMutableTreeNode rootNode = new DefaultMutableTreeNode(
                new NamePathModel("/oak:index/myProp", "/oak:index/myProp", store.getHead(), sizeCache,
    ...
    public void valueChanged(DefaultMutableTreeNode node) {
        if (node == null) {
            return;
        }
        // load child nodes:
...
//            System.out.println(sb.toString());
    ...
    public void printDiff(String input) {
        ...
        if (input == null || input.trim().length() == 0) {
            ...
            sb.append(input);
            sb.append(newline);
            System.out.println("Usage <recordId> <recordId> [<path>]");
            return;
        }
    ...
    private boolean safeRevert(String revision, boolean rollback) {
        ...
        try {
            refreshModel();
            if (!rollback) {
                System.out.println("Switched head revision to " + revision);
            }
