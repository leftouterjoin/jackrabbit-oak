...
リポジトリのサイズが管理不能なサイズに達するすると、セグメントの圧縮が必要となります。
リポジトリの書き込み操作が多いとき肥大化する可能性があり、かなりの量のガベージコレクションを取得する必要のあるコンテンツリビジョンが作成されているでしょう。
最も簡単な解決策は、`RevisionGarbageCollection`のMBean経由JMX操作として公開リビジョンガベージコレクションをスケジュールすることです。
これは、非ブロック操作で、それは次の`flush`呼び出しの間に圧縮内部フラグを設定します。
...
tool.  
サイズは既に大きすぎる場合には回避策として、[Oak run](https://github.com/apache/jackrabbit-oak/blob/trunk/oak-run/README.md)ツールを使用して手動圧縮を実行します。
